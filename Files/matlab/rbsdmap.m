...
%     end
    if isfield(cfg,'excludewfsrc')
        temp = cfg.excludewfsrc;
        badwfsrc = unique([temp find((sum(cfg.widesrc,2) == 0) | isnan(sum(cfg.widesrc,2)))']);
    elseif isfield(opt,'excludewfsrc')
        temp = jsonopt('excludewfsrc',[],opt);
        badwfsrc = unique([temp find((sum(cfg.widesrc,2) == 0) | isnan(sum(cfg.widesrc,2)))']);
    else
        badwfsrc = find((sum(cfg.widesrc,2) == 0) | isnan(sum(cfg.widesrc,2)))';
    end
...
%     end
    if isfield(cfg,'excludewfdet')
        temp = cfg.excludewfsrc;
        badwfdet = unique([temp find((sum(cfg.widedet,2) == 0) | isnan(sum(cfg.widedet,2)))']);
    elseif isfield(opt,'excludewfdet')
        temp = jsonopt('excludewfdet',[],opt);
        badwfdet = unique([temp find((sum(cfg.widedet,2) == 0) | isnan(sum(cfg.widedet,2)))']);
    else
        badwfdet = find((sum(cfg.widedet,2) == 0) | isnan(sum(cfg.widedet,2)))';
    end
